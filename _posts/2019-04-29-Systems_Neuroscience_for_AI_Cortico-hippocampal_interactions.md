...
_Neural computations are often compared to instrument-measured distance or duration, and such relationships are interpreted by a human observer. However, neural circuits do not depend on human-made instruments but perform computations_ 
...
If the hippocampus is where we instantiate or at least organise our imagination and novel thoughts as well as our recall, how do we choose what to call into imagination or to recall? We’re approaching a systems level of goal-directed behaviour, where we might consider ‘memory selection’ or ‘imagination selection’ as a behaviour that is optimised for by some policy. Before tackling the system that coordinates these functions (where we’ll look at the combination of the hippocampus, the thalamus, and the cortex) we should first address the system that coordinates goal-directed behaviour: the reinforcement learning system. 
